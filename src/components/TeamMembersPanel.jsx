...
async function sendNotification(userId, type, title, body) {
  ...
  if (prefs && prefs[`email_${type}`] === true) {
    ...
    if (recipientEmail) {
      ...
      const htmlBody = buildEmailHtml({ title, body, ctaLabel: 'View Invite in LabHive →', ctaUrl: 'https://labhive.app/?screen=profile&tab=team', prefsUrl: 'https://labhive.app/?screen=profile', orgContact })
